...
46. **Scenery asks last and takes nothing.** It is placed after the captions
...
47. **The river runs top to bottom, never along the board.** The axis is
    ...
    this map descends.
48. **It crosses rails and is not masked.** A rail over water is a bridge
    ...
    against nothing.
49. **It never crosses a word.** Grey behind letterforms is a smudge at any
    z-order. Text is solid to the solver; rails are not.
50. **Octilinear, like everything else.** 45 and 90 only, filleted at the
    same radius the rails use. A spline would be prettier water and a worse
    map.
51. **Scenery may be absent, and often should be.** A crowded day has no
    room for it, and no river beats one squeezed through two free cells.
52. **It is seeded from the date.** The same day draws the same water on
...
53. **Greys only.** A one-bit panel has no thirteen percent, so it gets no
