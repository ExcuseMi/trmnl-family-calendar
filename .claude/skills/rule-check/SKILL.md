...
- the instruction to read `issues.md`'s "Rules that are stated but not kept"
  equivalent at the end of `rules.md`, so it does not spend its report on
  the three gaps that are already written down,
