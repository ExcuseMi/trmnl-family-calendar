module.exports = function (test, h) {
  ...
  test('box_height_pct: the readable chip may grow into free room but never past the next same-lane event\'s real start', async () => {
    ...
    // well past its own tiny 15-min duration.
    assert(standup.box_height_pct > standup.height_pct * 2, 'a short event with free room after it should get a noticeably taller readable box than its bare duration: height=' + standup.height_pct + ' box=' + standup.box_height_pct);
  });
};
