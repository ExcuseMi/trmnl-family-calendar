...
module.exports = function (test, h) {
  ...
  const CONFIG = {
    ...
    calendars: [{ url: 'https://a.example/m.ics', rules: [{ match: { type: 'any' }, line: 'Mia' }] }],
  };
  ...
  test('renaming a line carries the rules that route to it', () => {
    ...
    assertEqual(out.calendars[0].rules[0].line, 'Mila',
  ...
  test('renaming past another line\'s name does not steal that line\'s rules', () => {
    const { document } = loaded({
      ...
      calendars: [
        { url: 'https://a.example/one.ics', rules: [{ match: { type: 'any' }, line: 'Al' }] },
        { url: 'https://a.example/two.ics', rules: [{ match: { type: 'any' }, line: 'Alex' }] },
      ],
    });
    ...
    assertEqual(out.calendars[1].rules[0].line, 'Alex',
      'the other line lost its own calendar to a rename that passed through its name');
    assertEqual(out.calendars[0].rules[0].line, 'Alexa',
