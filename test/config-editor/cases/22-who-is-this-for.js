...
module.exports = function (test, h) {
  ...
  test('two answers produce the configuration the plugin expects', () => {
    ...
    assertEqual(jsonOut(document), {
      ...
      calendars: [{
        ...
        rules: [{ match: { type: 'any' }, line: 'Homer' }],
      }],
    });
  ...
  test('a household is the same two answers, once per person', () => {
    ...
    cfg.calendars.forEach((c, i) => {
      assertEqual(c.rules, [{ match: { type: 'any' }, line: cfg.lines[i].name }],
  ...
  test('a name that differs only in case is the same person', () => {
    ...
    assertEqual(jsonOut(document).calendars[1].rules, [{ match: { type: 'any' }, line: 'Homer' }]);
  ...
  test('renaming a person carries their calendar and its rule along', () => {
    ...
    assertEqual(jsonOut(document), {
      ...
      calendars: [{
        ...
        rules: [{ match: { type: 'any' }, line: 'Homer J' }],
      }],
    });
  ...
  test('removing a person leaves nothing routing to them', () => {
    ...
    assertEqual(jsonOut(document).calendars[0], { url: 'https://calendar.example.com/homer.ics' },
      'the feed that was theirs still carries their name, which the plugin draws as a line');
  ...
  test('a person says which calendars their line is drawn from', () => {
    ...
    // Named by the file it fetches, not by the person: "From Homer" on
    // Homer's own card is a sentence that says nothing.
    assert(/From homer\.ics/.test(people(document)[0].textContent),
  ...
  test('a feed a couple shares is two names in the same box', () => {
    ...
    assertEqual(cfg.calendars[1], {
      ...
      rules: [{ match: { type: 'any' }, line: ['Marge', 'Homer'] }],
    }, 'a shared feed did not route to both of them, or was named after the pair');
    const parsed = window.parseConfig(document.getElementById('jsonOut').value);
    assertEqual(parsed.calendars[1].rules.length, 1, 'the plugin dropped the shared rule');
  });
  ...
  test('splitting a feed by rules is behind the card, and does not need the answer', () => {
    ...
    const adv = card.querySelector('details.adv');
    ...
    h.selectMulti(rule.querySelector('select[multiple]'), ['Bart']);
    const cfg = jsonOut(document);
    ...
    assert(/From 2 calendars: bart\.ics, school\.ics/.test(people(document)[0].textContent),
  ...
  test('a rule with nobody to route to says how to get somebody', () => {
    ...
    const sel = document.querySelector('#globalRules select[multiple]');
    assert(sel.disabled, 'the picker offers a choice of nobody');
