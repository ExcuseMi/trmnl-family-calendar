...
//   3. LET THE DATA ANSWER. A feed that looks like Nala's on Alex's screen is
...
module.exports = function (test, h) {
  ...
  test('the first question is who lives here, and the answers become the board', () => {
    ...
    fireInput(document.getElementById('wzName1'), 'Nala');
    click(next(document));
    ...
    assertEqual(jsonOut(document).lines, [{ name: 'Alex' }, { name: 'Nala' }]);
  ...
  test('a name taken off the list is taken off the board too', () => {
    ...
    const remove = [...document.querySelectorAll('#wzBody .wz-people button')]
      .find((b) => /Remove/.test(b.textContent) && /Nala/.test(b.getAttribute('aria-label') || ''));
  ...
  test('consent is asked again for the next link, because it is a different link', async () => {
    ...
    toFirstLink(document, ['Alex', 'Nala']);
    ...
    assert(/where is Nala's calendar/i.test(q(document)), 'it did not move on to the next person');
    // the second link asks all over again
    await refuse('https://cal.example.com/nala.ics');
  ...
  test('a calendar that looks like somebody else\'s is queried, not accepted', async () => {
    // The real mistake: two tabs open, and Nala's link pasted on Alex's screen.
    const { document } = loadEditor(direct(ics({
      name: 'Nala',
      events: [
        { t: 'Nala: Swimming', at: '20991103T160000' },
        { t: 'Nala: Piano', at: '20991104T170000' },
      ],
    })));
    toFirstLink(document, ['Alex', 'Nala']);
    ...
    click(h.buttonByText(document.getElementById('wzBody'), 'Check this link'));
    await h.flush();
    assert(/looks like Nala's calendar rather than Alex's/.test(body(document)),
    ...
    assert(/Nala/.test(next(document).textContent), next(document).textContent);
    click(next(document));
    assertEqual(jsonOut(document).calendars[0], {
      url: 'https://cal.example.com/whoops.ics', name: 'Nala',
      rules: [{ match: { type: 'any' }, line: 'Nala' }],
  ...
  test('one calendar for the whole house is written as nothing but its address', async () => {
    ...
    assertEqual([].concat(parsed.allLines), ['Alex', 'Nala'],
  ...
  test('public holidays are added as the day\'s, with no name and no line', async () => {
    ...
    click(belgium);
    assert(/holiday/.test(document.getElementById('wzUrl').value),
      'picking a country filled in nothing: ' + document.getElementById('wzUrl').value);
    click(h.buttonByText(document.getElementById('wzBody'), 'Check this link'));
    await h.flush();
    click(next(document));
  ...
  test('a calendar added by mistake can be removed without leaving the wizard', async () => {
    ...
    const remove = [...document.querySelectorAll('#wzBody .wz-row button')]
  ...
  test('carrying on puts the answers and the calendars back', async () => {
    const first = loadEditor(direct(ics({ name: 'Alex' })));
    toFirstLink(first.document, ['Alex', 'Nala']);
    fireInput(first.document.getElementById('wzUrl'), 'https://cal.example.com/alex.ics');
    click(h.buttonByText(first.document.getElementById('wzBody'), 'Check this link'));
    await h.flush();
    click(next(first.document));
  ...
  test('pasting back what TRMNL holds reopens the wizard with every answer in it', () => {
    ...
    document.getElementById('wzPaste').value = JSON.stringify({
      ...
      calendars: [
        { url: 'https://cal.example.com/alex.ics', name: 'Alex',
          rules: [{ match: { type: 'any' }, line: 'Alex' }] },
        { url: 'https://cal.example.com/nala.ics', name: 'Nala',
          rules: [{ match: { type: 'any' }, line: 'Nala' }] },
    ...
    const rows = [...document.querySelectorAll('#wzBody .wz-row')].map((r) => r.textContent);
    assert(rows.some((t) => /^Alex/.test(t) && /alex\.ics/.test(t)), 'Alex is not shown with her calendar: ' + rows);
    assert(rows.some((t) => /^Nala/.test(t)), 'Nala went missing');
    assert(rows.some((t) => /Everybody/.test(t) && /every line/.test(t)),
      'the shared calendar is not shown as the household\'s: ' + rows);
    assert(rows.some((t) => /Holidays/.test(t)), 'the holiday feed went missing: ' + rows);
  ...
  test('the last screen is one copy button, with the text behind a disclosure', async () => {
    ...
    const t = body(document);
    ['Use Demo Data', 'Calendars', 'Location', 'Save'].forEach((phrase) => {
