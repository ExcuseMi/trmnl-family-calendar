...
- `color`: a hue name (`blue`, `green`, `orange`, `purple`, `red`, `cyan`,
  ...
  work-side line is black, the others cycle hues.
- `side`: `"work"` (or `"left"`) puts the line on the work side of the spine
  (top in horizontal layouts, left in vertical); `"family"`/`"right"` the
  other side. Without it, the person a calendar named **Work** assigns goes
  on the work side and everyone else on the family side.
