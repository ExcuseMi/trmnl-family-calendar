...
* **Core Rule:** Render events as **pure typographic signage** anchored by continuous, single-stroke SVG line paths.
...
* Two sources. The board rides the demo whenever **Calendars** names nothing to draw, which is what a fresh install is; **Force Demo Data**, under **Developer**, off by default, runs it over real feeds anyway. Either way that is the hardcoded demo events (no network) — a day in Springfield: Homer, Marge, Bart & Lisa and Maggie, with the plant and the school as sidings, or a pasted **Calendars** config — same shape as this repo's `calendar-config.json`/`demo-config.json` — fetched as real ICS and parsed with a hand-rolled parser (no ICS library available in Serverless).
...
* **Demo mode** runs `DEMO_CONFIG` (in transform.js) through the real fetch/parse/rule pipeline against the ICS files in `demo/` on this repo's `main`, so the configuration every new device starts on is also a worked example of the config format — and is covered. `test/transform/cases/demo-config.js` serves those files from disk. There is no offline fallback: a hardcoded Springfield day used to stand in when GitHub was unreachable, and it was a second, drifted copy of the same board (different titles, one hardcoded day where the feeds recur, and the only hand-pinned `side`/`line_offset` in the plugin, so the ordering solver never ran on it) that swapped itself in silently. A device that missed the fetch budget showed a different board with no sign anything was wrong. When the feeds do not answer the board draws the five declared lines with nothing on them, which says who is missing and is obviously not a working day. Note the ICS files must be on `main` for a live device to see them.
* The **rule engine** (is covered by `test/transform/` (`npm test` there, no Docker): match types `any`/`all`, `exact`, `word`, `contains`, `regex`, `status`, `weekday`, `and`/`or`, `not` (`{matcher: Matcher}`, negates it — prefer this over a regex negative lookahead for "one of these except that one," since it needs no backslash escaping to get wrong); actions `line` (string or list), `rename` (defaults true, but false on an any/all match), `rewrite`/`rewriteFull`, `hide`, `allDay`; top-level `rules` run before each calendar's own (calendar wins ties); the first `lines[]` entry is the `everyoneLine` fallback; calendars may be bare URL strings; non-JSON config text is a newline-separated URL list; per-calendar `headers` go on the fetch. A `line` list becomes an interchange (`owner` + `co_owners`). Per-calendar `includeDescription: true` is the only thing that makes DESCRIPTION get parsed and matched — off by default.
* **The forecast covers a run of days; the board draws one of them, at a particular minute.** Every fact taken out of the snapshot has to be indexed by the day it describes and, when it names an hour, held to the clock. This has gone wrong four separate ways: the wettest hour scanned across both days, so tomorrow's rain became today's service alert; the rain markers ran on into day 1 as soon as day 0 had fewer than two crossings, carrying "it is raining" over the midnight gap and drawing a "Rain Stops 07:00" before the "Rain Starts 13:00" it belonged to; sunrise/sunset (since removed) were a hardcoded `[0]`; and `now_min` was passed through on a board showing tomorrow, parking a car on every line at a minute nobody had reached. The snapshot now records the civil day of its own day 0 (`snap.date`) and `buildFromConfig` slides the index by the days elapsed, so a forecast fetched at 23:30 and read at 04:00 is read as the yesterday it is. A banner is a promise about what is COMING: `wettestHour(hours, from)` re-picks the hour at draw time, the hour happening right now still counts, and a day that is not today gets no clock at all. Tests: `service-alert.js` ("Nothing in the past") and `multi-day.js` ("The sky band, the clock, and the day they belong to").
* Lines are **dynamic** in config mode, not the fixed 3 from demo data — discovered from `config.lines` (a `color` there overrides the auto hue cycle) plus any calendar whose events resolve to no rule-assigned line (that calendar's own name becomes a fallback line, so nothing silently vanishes). Side (left/right) is decided once every calendar is fetched and every event tallied: an explicit `line.side` wins, everyone else is balanced by their own event count (heaviest first, each to whichever side is currently lighter) — not by any calendar's name.

---

## Layout (see ../DESIGN.md for the full algorithm)

* **All geometry is client-side** in `shared.liquid`'s `<script>`: `transform.js` returns raw facts only (events with `start_min`/`end_min`, `owner`/`co_owners`, `side`, `hue`, `line_offset`, `line_width`, `line_style`; weather milestones with `at_min`; `now_min`, `date_label`, `orientation`). No pixel or percentage positions come from the backend — it can't know the canvas size, the zoom factor, or how wide a title renders.
...
* **Bands:** each line gets its own stretch of the cross-axis — its baseline, room for its siding kink, then its OWN lanes — so a branch never reaches across a neighbouring line and cannot cross one. Band sizes come from what each line's events actually need (measured by placing that line alone first); leftover room goes to lane pitch, then to separation between lines, both capped, and the rest is centred. When the bands cannot fit — the 800x480 panel with a few all-day kinks — it falls back to the original packed bundle with one lane pool shared per side. Interchanges are a thin tie between the lines with a ring on each, not one capsule stretched across the gap; a station junction is a hollow diamond.
* **Bands:** every line gets its own stretch of the cross-axis — baseline, siding kink room, then its OWN lanes, sized to what its events need and spread over the canvas. An event may only take a lane rung its own line owns, so a branch can't reach across a neighbour (an interchange takes its rung from the OUTERMOST line it joins, where its ring physically sits). When bands can't fit — the 800x480 panel with a few all-day kinks — it falls back to the old packed bundle with one shared lane pool per side.
...
* **Spine:** every line's line runs full-bleed along the axis in one bundle;
...
* **Shared sidings draw their lines TOGETHER.** An ordinary siding kinks a line AWAY from the spine; a shared one kinks its members toward each other (inner steps out, outer steps in) so they run alongside each other for the length of the thing they are both at, with a bar across them at each end of the span. Two people at the same school kinking away from each other read as two unrelated sidings that happened to share a name. Direction comes from board order (`line_offset`), not cross positions, because those are still being solved when `sidingRaiseAt` is read; that function returns a SIGNED delta now. Members on opposite sides of the spine keep the ordinary outward kink — converging across the middle would cross the hour river.
...
* **One event, drawn once:** two calendars can describe the SAME thing (Bart's "L6 School Day" and Lisa's "L2 School Day" both rename to "School Day" over the same hours). `mergeAcrossLines` in transform.js folds anything with the same title over the same minutes into one event with `co_owners`. A merged STATION still emits one entry per owner — both children really are at school, so both lines kink — sharing a `group` id so the client draws the caption once, set midway between the lines it belongs to. Both the merge and a rule-assigned line list feed `registry.link()`.
* **Line order:** the lines are laid out as ONE chain, strongest shared-event link first, extended at whichever end offers the next strongest. The board is a chain too (outermost left … innermost left, spine, innermost right … outermost right), so a chain laid along it keeps every consecutive pair adjacent and can be cut anywhere. The cut is chosen to balance the two sides and to fall on a weak link; an explicit `side` rules out any cut that contradicts it. The anchor line (black, boldest, solid) is the first line the config names, else the busiest.
* **A quiet day borrows the next one (the 36-hour view).** At or below `QUIET_DAY_MAX_EVENTS` distinct events on the day being SHOWN (counted after rules, hides and the cross-calendar merge, all-day states excluded), `buildFromConfig` rebases two days instead of one and sends `rolling: {start_min, end_min}`, the window into that run (06:00 of the shown day to 18:00 of the next, widened at either end so no event it keeps is cut). Everything is still rebased onto the shown day, so minute zero means the same thing it always did; `show_day` still picks WHICH day, and a board about tomorrow gathers the day after so it has one to borrow. The count is per civil day and the window is anchored to the day, never to `now`: the panel refreshes every fifteen minutes and the board may not change shape under a reader. Off with the **Quiet Days** setting. On the client, `TimeAxis.fit` takes that window and clips the same compression profile to it, and falls back to the ordinary single day where the run would leave an unreadable hour or where the borrowed day has nothing in the window. A busy board's payload is byte-identical to before apart from `rolling: null`.
...
* **Long blocks** (a school day, a shift, a desk booking): recognised by LENGTH, never declared. One person's block is `_onLine` (`staticFacts` in shared.liquid): no rail of its own and no lane, drawn along the stretch of that person's own line the block covers, with a thin leader from its caption back to the line. A block several people share is a convergence instead, their lines running together for its span. The meetings inside either still branch off normally, via `e._lineDist`/`e._lineDistEnd`. The old `siding: true` config key and `metro.sidings` payload are both gone; a config still carrying the key is read as if it did not.
...
* **Line names sit above their line only when there is room above it**, decided after the bands are solved from the real line pitch; where the lines end up closer together than a name is tall, each name lands on its neighbour's ("Lisa" and "Marge" came out as one word), so they sit ON their own line instead and their paper outline masks it.
* **Weights come down with the pitch** (`weightScale`). Line widths are tuned for a board where every line has a band; on a panel where they end up a line-step apart, a 6px line is a third of the gap to the next one and the map turns into stripes.
* **Marks are sized off the RAIL, not off a constant** (`markOut`). At a fixed radius the start dot vanished into a heavy line and stood clear of a light one, so the same mark read as a knob on one line and a bar on another.
...
* **Line names:** a terminus column is reserved out of the start of the axis and every line begins there with its own name, at its own height, capped by a terminal bar at each end. There is no legend and no lettered-bullet fallback — the name is on the line rather than in a key.
* **Payload shape:** `transform.js` returns `{ data, trmnl_state }`, and a serverless transform's returned keys ARE the template's root variables, so every template path starts `data.`. Inside it, `data.events` is the day's events and `data.weather` is the sky markers — rain start/stop and the heavier conditions, one band on the board. Sunrise and sunset used to ride in the same list under their own `type`; they were removed from the board, so `weather` now holds one kind. `data.header_weather` is separately the header's. These were one mixed `items` list sorted by minute, and every consumer's first move was to filter it by type, so the mixing was work done once here and undone four times downstream.
* **Sky band:** rain start/stop markers (icon + text) sit in a band along the top edge in horizontal mode, beside the bundle in vertical mode.
...
* **All-day events:** an all-day event has no hour, so it gets no place on a scale of hours. It is declared once at the line's head, in a second row under the name with the concentric rings a transit map gives its biggest stations, and both ends of that line become open chevrons because the day is a slice of something longer. One title shared by several lines is one origin, named once, with a dashed tie down to the other heads. Nothing is drawn for it between the first hour and the last.
* **Layout tests:** see "Testing the layout" below. `test/layout/` (`npm test` there) renders the real built page in headless Chromium against fixture METRO payloads and asserts geometry invariants. Elements carry `data-metro-role` (`line`/`branch`/`fork`/`capsule`/`ring`/`station-ring`/`stop`/`bullet`/`now`/`car`) purely so tests can tell them apart — keep them on anything new that gets drawn.
...
* **Local preview:** `.trmnlp.yml` mirrors the demo data (transform runtime disabled) — regenerate it after changing `DEMO_EVENTS`/`DEMO_STATIONS`/`DEMO_ALLDAY` (command in the file's header). `trmnlp lint` must come back clean before a push. Two of its checks need
knowing about: it counts the STRINGS `padding`, `margin`, `font-size` and five
