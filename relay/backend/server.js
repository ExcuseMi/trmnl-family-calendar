...
//
// PER HOUR, AND SIZED FOR A HOUSEHOLD. The editor asks for every unread
// feed at once, so one press of the button is one request per calendar: a
// family with nine of them spends nine. A cap of ten would lock somebody
// out of their own board on the second press, which reads as the relay
// being broken. Thirty is three passes over a nine-calendar config and
// still nothing like a useful amount of somebody else's bandwidth. Set
// RATE_MAX to tighten it.
const RATE = {
  windowMs: Number(process.env.RATE_WINDOW_MIN || 60) * 60 * 1000,
